...
export async function scrapeIndieHackers(): Promise<Product[]> {
  const browser = await puppeteer.launch({
    headless: process.env.CI === "true" ? true : false, // CI 환경에서는 headless 모드 사용
    args: [
      "--no-sandbox",
...
/**
 * 보고서 생성을 위한 제품 정보를 스크래핑합니다.
 * 웹사이트 스크래핑 없이 IndieHackers 페이지만 분석합니다.
 * @param maxCount 최대 가져올 제품 개수 (기본값: 50)
 */
export async function scrapeProductsForReport(maxCount: number = 50): Promise<Product[]> {
  const browser = await puppeteer.launch({
    headless: process.env.CI === "true" ? true : false, // CI 환경에서는 headless 모드 사용
    args: [
      "--no-sandbox",
  ...
  console.log("🌐 IndieHackers /products 페이지 로딩 중...");
  try {
    await page.goto("https://www.indiehackers.com/products", {
      waitUntil: "networkidle2",
      timeout: 120000,
  ...
  console.log("🔍 /products 페이지에서 제품 리스트 수집 중...");
  ...
  console.log(`📦 기본 정보 수집 완료: ${productData.length}개`);
  console.log("🔍 각 제품의 /product/detail 페이지 분석 중...");

  const products = await Promise.all(
    productData.slice(0, maxCount).map(async (product) => {
      let thumbnail = "";
      let actualRevenue = product.revenue;
      let firstFeedPost = "";
      let isStripeVerified = product.isStripeVerified || false;
